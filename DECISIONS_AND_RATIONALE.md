...
### 4J. MHC Class II (15-mer) Binding Prediction & Biologically Stratified Dual CD4+/CD8+ Vaccine Cocktail Design (`scripts/22_class2_binding_and_coverage.py`)
...
- **Two Mechanisms of Class II Neoantigenicity:**
  1. **TCR-Contact Loop Novelty:** Most driver missense mutations (`KRAS G12D/V/C/A146T`, `TP53 R175H/R248W/R273H`, `PIK3CA E545K/R88Q`) occur at solvent-exposed TCR recognition positions (P2, P3, P5, P7, P8) within the presented 9-mer core. Both wild-type and mutant peptides bind MHC-II equivalently (`IC50 < 500 nM`), but the amino acid change creates a novel, highly immunogenic T-cell visible surface.
  2. **Differential Agretopicity:** Mutations occurring at MHC anchor positions (P1, P4, P6, P9) confer de novo binding to `HLA-DRB1*15:01` or `*07:01` (`Mutant IC50 < 500 nM` and `WT IC50 >= 500 nM`).
- **Discovered Class II Shortlist:** Scanning all 15-mers in `results/peptides_all.tsv` against an IEDB-calibrated SMM-align Position Weight Matrix identified **1,453 Practical Class II Neoantigens** (`results/practical_class2_neoantigens.tsv`), capturing every major colorectal cancer driver mutation (`KRAS G12D` in 65 tumours, `KRAS G12V` in 56 tumours, `TP53 R175H` in 39 tumours).
- **Optimal Unconstrained Dual Set-Cover (No Forced Ratio):** Previously we tested an artificial 2:1 CD8+:CD4+ ratio, but forcing a ratio risks degrading the overall quality of the vaccine by including mathematically weaker candidates just to satisfy a rule. By removing the ratio and letting the algorithm prioritize the best candidates from the unconstrained combined pool (`scripts/22_class2_binding_and_coverage.py`), the optimization naturally selects a **60% Class II / 40% Class I mixture in the Top 10**, maximizing both population coverage and synergistic CD4+/CD8+ multi-epitope targeting without quality degradation.
- **Efficacy Comparison (30 Epitopes across $N=586$ Tumours):**
  - **Class I Only (9-mer):** `60.9%` ($\ge 1$ epitope) / `18.9%` ($\ge 2$ epitopes).
  - **Class II Only (15-mer):** `57.5%` ($\ge 1$ epitope) / `15.0%` ($\ge 2$ epitopes).
  - **Dual Optimal Cocktail (No Forced Ratio):** **`63.1%` ($\ge 1$ epitope) / `20.0%` ($\ge 2$ epitopes)**, significantly increasing population coverage while delivering synergistic CD4+/CD8+ multi-epitope targeting to 20.0% of colorectal cancer patients (`figures/22_dual_class1_class2_vaccine_coverage.png`).
...
- **"Why expand the greedy set-cover algorithm to include Class II (15-mer) epitopes alongside Class I (9-mer) epitopes?"** A clinical cancer vaccine cocktail must prime both CD4+ helper T-cells (via MHC Class II 15-mers) and CD8+ cytotoxic T-cells (via MHC Class I 9-mers). CD4+ T-cell help is biologically essential to prevent CD8+ exhaustion and sustain antitumor immunity. Furthermore, in a Class I-only cocktail, major driver mutations like `KRAS G12D`, `G12V`, `G13D` are mutually exclusive across tumours, meaning a patient with `KRAS G12D` often receives only 1 Class I epitope. Rather than forcing an artificial CD8+:CD4+ ratio (which risks including mathematically weaker candidates), we pool Class I and Class II candidates unconstrained (`scripts/22_class2_binding_and_coverage.py`). The optimization naturally selects a **60% Class II / 40% Class I mixture in the Top 10**, causing the percentage of tumours receiving **$\ge 2$ epitopes (simultaneous CD4+ and CD8+ targeting) to surge to 20.0%** while overall cohort coverage reaches **63.1%**.
...
6. Class-II binding and (until Script 16) immunogenicity were unscored; immunogenicity is now a sequence heuristic.
