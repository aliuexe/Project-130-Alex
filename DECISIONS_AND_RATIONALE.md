...
- **Stratified Dual Set-Cover (2:1 CD8+:CD4+ Ratio):** In a Class I-only cocktail, `KRAS G12D`, `G12V`, and `G13D` are mutually exclusive across tumours, so a patient with `KRAS G12D` typically receives only 1 epitope. By implementing a biologically stratified 2:1 CD8+:CD4+ set-cover algorithm (`scripts/22_class2_binding_and_coverage.py`), patients simultaneously receive a cytotoxic Class I target and a complementary Class II helper target.
- **Efficacy Comparison (30 Epitopes across $N=586$ Tumours):**
  - **Class I Only (9-mer):** `60.9%` ($\ge 1$ epitope) / `18.6%` ($\ge 2$ epitopes).
  - **Class II Only (15-mer):** `57.5%` ($\ge 1$ epitope) / `15.2%` ($\ge 2$ epitopes).
  - **Dual Stratified Cocktail:** **`63.1%` ($\ge 1$ epitope) / `20.0%` ($\ge 2$ epitopes)**, significantly increasing population coverage while delivering synergistic CD4+/CD8+ multi-epitope targeting to 20.0% of colorectal cancer patients (`figures/22_dual_class1_class2_vaccine_coverage.png`).
